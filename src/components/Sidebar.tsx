...
export function Sidebar() {
  const {activeTool, selectedMarker, 
         ...
         midspans,
         viewer, poles,
         setMidspansMeasurements, midspanMeasurements,
  ...
  const handleAddNewPrimary = (selectedMarker) => {
    ...
    if (!selectedMarker.pole1) {
      ...
      setMidspanMeasurements(prevMeasurements => {
        // Create a new array with the updated measurement
        const updatedMeasurements = [...prevMeasurements];
        updatedMeasurements[index] = {
          ...updatedMeasurements[index], // Copy the existing measurement
  ...
  function confirmMeasurement(kind, markerId, index, type) {
  ...
  return (
    <div className="w-1/5 shrink-0 bg-primary-foreground h-full text-sm overflow-auto">
        { selectedMarker && selectedMarker?.pole1 && (
          <div className="px-4 py-2 border-b">
            midspan {selectedMarker.id + 1}
          </div>
        )}
        { selectedMarker && !selectedMarker?.pole1 && (
          <div className="px-4 py-2 border-b">
        ...
        {selectedMarker && selectedMarker?.midpoint ? (
          <>
          <div className="px-4 py-2 border-b">
          <span>Lat/Long:    </span><span className="text-xs">{selectedMarker.midpoint.lat} / {selectedMarker.midpoint.lng}</span>
          </div>
          </>
        ) : selectedMarker ?(
          <>
            ...
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {primary === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('primary', selectedMarker.id, index, 'pole')}/>
          ...
          {measurements[selectedMarker.id]?.neutrals?.map((neutral, index) => (
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {neutral === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('neutral', selectedMarker.id, index, 'pole')}/>
                  <Cancel setActiveTrace={setActiveTrace} setActiveMeasurement={setActiveMeasurement}/>
                </div>
              ): (
                <>
          ...
          {measurements[selectedMarker.id]?.secondaries?.map((secondary, index) => (
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {secondary === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('secondary', selectedMarker.id, index, 'pole')}/>
                  <Cancel setActiveTrace={setActiveTrace} setActiveMeasurement={setActiveMeasurement}/>
                </div>
              ): (
                <>
          ...
          {measurements[selectedMarker.id]?.transformers?.map((transformer, index) => (
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {transformer === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('transformer', selectedMarker.id, index, 'pole')}/>
                  <Cancel setActiveTrace={setActiveTrace} setActiveMeasurement={setActiveMeasurement}/>
                </div>
              ): (
                <>
          ...
          {measurements[selectedMarker.id]?.driploops?.map((driploop, index) => (
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {driploop === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('driploop', selectedMarker.id, index, 'pole')}/>
                  <Cancel setActiveTrace={setActiveTrace} setActiveMeasurement={setActiveMeasurement}/>
                </div>
              ): (
                <>
          ...
          {measurements[selectedMarker.id]?.risers?.map((riser, index) => (
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {riser === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('riser', selectedMarker.id, index, 'pole')}/>
                  <Cancel setActiveTrace={setActiveTrace} setActiveMeasurement={setActiveMeasurement}/>
                </div>
              ): (
                <>
          ...
          {measurements[selectedMarker.id]?.comms?.map((comm, index) => (
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {comm === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('comm', selectedMarker.id, index, 'pole')}/>
                  <Cancel setActiveTrace={setActiveTrace} setActiveMeasurement={setActiveMeasurement}/>
                </div>
              ): (
                <>
        ...
        <div className="flex flex-col gap-2 px-4 py-2 border-b">
          ...
          {/* PRIMARIES */}
          {midspanMeasurements[selectedMarker.id]?.primaries?.map((primary, index) => (
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {primary === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('primary', selectedMarker.id, index, 'midspan')}/>
                  <Cancel setActiveTrace={setActiveTrace} setActiveMeasurement={setActiveMeasurement}/>
                </div>
              ): (
                <>
          ...
          {/* Neutrals */}
          {midspanMeasurements[selectedMarker.id]?.neutrals?.map((neutral, index) => (
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {neutral === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('neutral', selectedMarker.id, index, 'midspan')}/>
                  <Cancel setActiveTrace={setActiveTrace} setActiveMeasurement={setActiveMeasurement}/>
                </div>
              ): (
                <>
          ...
          {midspanMeasurements[selectedMarker.id]?.secondaries?.map((secondary, index) => (
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {secondary === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('secondary', selectedMarker.id, index, 'midspan')}/>
                  <Cancel setActiveTrace={setActiveTrace} setActiveMeasurement={setActiveMeasurement}/>
                </div>
              ): (
                <>
          ...
          {midspanMeasurements[selectedMarker.id]?.comms?.map((comm, index) => (
            <div key={index} className="flex items-center justify-between pl-4 py-1 italic">
              {comm === activeTrace ? (
                <div className="flex w-full justify-between">
                  <WaitingDots />
                  <Check className="w-5 h-5 cursor-pointer" onClick={() => confirmMeasurement('comm', selectedMarker.id, index, 'midspan')}/>
                  <Cancel setActiveTrace={setActiveTrace} setActiveMeasurement={setActiveMeasurement}/>
                </div>
              ): (
                <>
